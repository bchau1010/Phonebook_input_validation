...
#USE PARAMETERIZED QUERIES FOR SQL
    # using parameter in statement instead of concatenation
# 2 group of input, syntax check for format, sematic check for appropiation
# check for bad data, reject it if it invalid, do not try to repair it

# white listing rule using regular expression, missing length check and integer overflow
    # integer overflow: use max and min bound before using it
# Use to stop command injection and path manipulation
name_regex = re.compile(r"^[a-zA-Z]+(?: [a-zA-Z]+)*(?:, [a-zA-Z]+(?: [A-Z]\.)?)?$")
phone_regex = re.compile(r"^(\+?[1-9]{1,4})?[-.\s]?(\(?\d{1,4}\)?[-.\s]?\d{1,4}[-.\s]?\d{1,4})$")
...
#get the list of phone numbers in database
